...
% L_T
FunObj.L_T = Function('L_T', {OCPEC.x, OCPEC.u, OCPEC.p, OCPEC.w}, {OCPEC.L_T},...
    {'x', 'u', 'p', 'w'}, {'L_T'});
% L_S
...
% G
G_FunObj = Function('G',{OCPEC.x, OCPEC.u, OCPEC.p, OCPEC.w}, {OCPEC.G},...
    {'x', 'u', 'p', 'w'}, {'G'});
...
% F
F_FunObj = Function('F', {OCPEC.xPrev, OCPEC.x, OCPEC.u, OCPEC.p, OCPEC.w}, {OCPEC.F},...
    {'xPrev', 'x', 'u', 'p', 'w'}, {'F'});
FunObj.F = F_FunObj.map(nStages);
% PHI
PHI_FunObj = Function('PHI', {OCPEC.x, OCPEC.u, OCPEC.p, OCPEC.w, OCPEC.s}, {OCPEC.PHI},...
    {'x', 'u', 'p', 'w', 's'}, {'PHI'});
...
Lu_T_formula = jacobian(OCPEC.L_T, OCPEC.u);
Lp_T_formula = jacobian(OCPEC.L_T, OCPEC.p);
Lw_T_formula = jacobian(OCPEC.L_T, OCPEC.w);
FunObj.L_T_grad = Function('L_T_grad',...
    {OCPEC.x, OCPEC.u, OCPEC.p, OCPEC.w},...
    {Lx_T_formula, Lu_T_formula, Lp_T_formula, Lw_T_formula},...
    {'x', 'u', 'p', 'w'},...
    {'Lx_T', 'Lu_T', 'Lp_T', 'Lw_T'});
% L_S_grad
...
Gw_formula = jacobian(OCPEC.G, OCPEC.w);
G_grad_FunObj = Function('G_grad',...
    {OCPEC.x, OCPEC.u, OCPEC.p, OCPEC.w},...
    {Gx_formula, Gu_formula, Gp_formula, Gw_formula},...
    {'x', 'u', 'p', 'w'},...
    {'Gx', 'Gu', 'Gp', 'Gw'});
...
Fw_formula = SX(Dim.x, Dim.w);
F_grad_FunObj = Function('F_grad',...
    {OCPEC.x, OCPEC.u, OCPEC.p, OCPEC.w},...
    {Fx_formula, Fu_formula, Fp_formula, Fw_formula},...
    {'x', 'u', 'p', 'w'},...
    {'Fx', 'Fu', 'Fp', 'Fw'});
FunObj.F_grad = F_grad_FunObj.map(nStages);
% PHI_grad
PHIx_formula = jacobian(OCPEC.PHI, OCPEC.x);
PHIu_formula = jacobian(OCPEC.PHI, OCPEC.u);
PHIp_formula = jacobian(OCPEC.PHI, OCPEC.p);
PHIw_formula = jacobian(OCPEC.PHI, OCPEC.w);
PHI_grad_FunObj = Function('PHI_grad',...
    {OCPEC.x, OCPEC.u, OCPEC.p, OCPEC.w, OCPEC.s},...
    {PHIx_formula, PHIu_formula, PHIp_formula, PHIw_formula},...
    {'x', 'u', 'p', 'w', 's'},...
    {'PHIx', 'PHIu', 'PHIp', 'PHIw'});
...
Fw = SX.sym('Fw', Dim.lambda, Dim.w);

PHIx = SX.sym('PHIx', Dim.gamma, Dim.x);
PHIu = SX.sym('PHIu', Dim.gamma, Dim.u);
PHIp = SX.sym('PHIp', Dim.gamma, Dim.p);
PHIw = SX.sym('PHIw', Dim.gamma, Dim.w);

HxT = Lx' - Gx'*sigma + Cx'*eta + Fx'*lambda - PHIx'*gamma;
HuT = Lu' - Gu'*sigma + Cu'*eta + Fu'*lambda - PHIu'*gamma;
HpT = Lp' - Gp'*sigma + Cp'*eta + Fp'*lambda - PHIp'*gamma;
HwT = Lw' - Gw'*sigma + Cw'*eta + Fw'*lambda - PHIw'*gamma;

HAM_grad_FunObj = Function('HAM_grad',...
    {sigma, eta, lambda, gamma,...
    Lx, Lu, Lp, Lw, Gx, Gu, Gp, Gw, Cx, Cu, Cp, Cw,...
    Fx, Fu, Fp, Fw, PHIx, PHIu, PHIp, PHIw},...
    {HxT, HuT, HpT, HwT},...
    {'sigma', 'eta', 'lambda', 'gamma',...
    'Lx', 'Lu', 'Lp', 'Lw', 'Gx', 'Gu', 'Gp', 'Gw', 'Cx', 'Cu', 'Cp', 'Cw',...
    'Fx', 'Fu', 'Fp', 'Fw', 'PHIx', 'PHIu', 'PHIp', 'PHIw'},...
...
J(1 : Dim.Node(4), Dim.Node(4) + 1 : Dim.Node(8)) = [-Gvar; Cvar; Fvar; -PHIvar];
J(Dim.Node(4) + 1 : Dim.Node(8), :) = [-Gvar', Cvar', Fvar', -PHIvar', Hessian + nu_H * SX.eye(Dim.Z)];

J_FunObj = Function('J',...
    {nu_G, nu_J, nu_H,...
    PSIgSigma_diagVec, PSIgG_diagVec, PSIphiGamma_diagVec, PSIphiPHI_diagVec,...
    Gx, Gu, Gp, Gw, Cx, Cu, Cp, Cw,...
    Fx, Fu, Fp, Fw, PHIx, PHIu, PHIp, PHIw,...
    Hessian},...
    {J},...
    {'nu_G', 'nu_J', 'nu_H',...
